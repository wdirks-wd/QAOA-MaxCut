...
exp_folder = v.create_fa_exp_folder(output_file_path, degree, num_nodes, max_depth, master_seed)
...
for node in range(num_nodes):
    ...
    c_name = f"node = {cur_node}"

    base_graph = nx.random_regular_graph(degree, cur_node, graph_seed)
    ...
    for depth in range(max_depth):
        ...
        column_name = f"depth = {depth}"

        # Prepare results storage dictionary
        results_data = {}

        # Unweighted Base Experiment
        print(f"({cur_node}),({depth+1}) Running Unweighted Experiment...")
        ...
        depth_results[column_name] = results_data

        df_means = pd.concat(mean_array, axis = 1)
        df_stds = pd.concat(std_array, axis = 1)
        df_stderrs = pd.concat(stderr_array, axis = 1)

    df_means.to_csv(f'{exp_folder}/nodes={cur_node}-output_file.csv', index=False)    
    overall_results[c_name] = depth_results
    ...
    ax.set_title(f"QAOA Approximation Ratio vs Depth for {cur_node} Nodes, {num_samples} Samples")
    ax.set_xticks(depths)
    ax.set_ylim(0, 1.1)
    ax.legend()
    ax.grid(True, linestyle='--', alpha=0.5)
    plt.savefig(f"{exp_folder}/Nodes = {cur_node} AR vs. Depth.pdf")
    plt.close()

df_overall = pd.DataFrame(overall_results)
#print(df_overall)
df_overall.to_csv(f'{exp_folder}/overall-output.csv')
...
print(df_array)
df_means.to_csv('output_file.csv', index=False)
